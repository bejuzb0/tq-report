...
and llama.cpp discussions~\cite{ggmlorg2026} note that memory savings alone are insufficient:
...
they reinforce the same core point: a theoretically appealing quantizer must be evaluated in the
downstream operation where it will be used. None of these works evaluate cross-modal retrieval tasks.
Same-modal nearest-neighbor benchmarks cannot reveal whether compression damages the inter-modal
alignment that text-to-image and image-to-text retrieval depends on, which is exactly the gap this
work addresses.
