...
Compressing those embeddings is the obvious fix, but most methods either require expensive training or
...
The catch is that all three were evaluated only on text embeddings or LLM activations. CLIP embeddings
...
depends on this alignment surviving compression intact, and it is not obvious that compressors designed
for isotropic text-like distributions will preserve it. We implement all three from paper specifications
and evaluate them on CLIP, making this the first study of the TurboQuant family on multimodal embeddings.
...
Because TurboQuant is a very recent paper (ICLR 2026), open-source implementations and benchmarks are
still emerging. The \texttt{tonbistudio/turboquant-pytorch}~\cite{tonbistudio2026} project provides a
from-scratch PyTorch implementation and documents settings where QJL residuals can be delicate,
particularly in generation workloads. Memoriant's DGX Spark KV-cache benchmark~\cite{memoriant2026}
and llama.cpp discussions~\cite{ggmlorg2026} emphasize that memory savings alone are insufficient:
dequantization and attention-time overheads can dominate latency at long context lengths. The
MLX-VLM project~\cite{mlxvlm2026} contributed a production-oriented TurboQuant reference using
Subsampled Randomized Hadamard Transform (SRHT) preconditioning in place of plain Gaussian projections.
These findings are complementary to ours. They study LLM inference while we study CLIP retrieval, but
they reinforce the same core lesson: a theoretically appealing quantizer must be evaluated in the
downstream operation where it will be used. Crucially, none of these works evaluate cross-modal
retrieval tasks. Same-modal nearest-neighbor benchmarks cannot reveal whether compression damages the
inter-modal alignment that text-to-image and image-to-text retrieval depends on, which is exactly the
...
and 155{,}070 captions with five captions per image. Images and captions are embedded with frozen CLIP
ViT-B/32 into 512-dimensional vectors and L2-normalized to unit norm (required by the polar decomposition
...
T1 and T2 are cross-modal tasks and serve as the primary stress test. T3 and T4 are same-modal controls.
We report Recall@10 as the primary metric: it is less noisy than Recall@1 while still sensitive to ranking
degradation under compression. Queries always remain at full float32 precision; only the database is
compressed (asymmetric retrieval).
...
\]
SRHT whitens CLIP's non-isotropic coordinate distribution so every sign bit is equally informative, unlike a plain Gaussian projection where low-variance dimensions waste bits.
...
We evaluate bit budgets $\{1, 2, 4\}$ for the analytic methods and $\{1, 2, 4, 8\}$ for FAISS-PQ, with float32 as the uncompressed baseline. Each configuration is run over five random seeds (0--4) to account for the randomness in projections, rotations, and codebook initialization; we report seed means with standard deviation as error bars. Before running CLIP experiments, we validated all compressors on synthetic unit vectors, confirming that inner-product correlation with ground truth improves monotonically with bit-width across all methods.
...
vs.\ $[0,\pi/2]$ at deeper levels. Cross-modal Recall@10 at 2\,bits improves from 0.349 to 0.406 (T1) and 0.548 to 0.629 (T2).
...
orthogonal rotation brings both close to zero kurtosis, which is what validates the theoretical angle
PDFs used by PolarQuant and TurboQuant. However, rotation preserves all distances and angles, including
the modality gap, which is why it helps with quantizer design but cannot fix the cross-modal fragility
problem.

%-----------------------------------------------------------------------
\section{Discussion}

\paragraph{Why does FAISS-PQ stay on top?}
It knows the data. At 1--2\,bits, there are so few bits that every single one needs to capture
dataset-specific structure, and a codebook trained on the actual CLIP distribution does this better
than any data-oblivious method. The price is a training pass and the need for representative data
upfront. If you can afford that, FAISS-PQ is the right call.

\paragraph{Why does PolarQuant work so well without training?}
Two things click together. First, random orthogonal rotation brings CLIP's non-Gaussian coordinate
distribution close to the angle distributions that PolarQuant's codebooks are derived for. Second,
polar decomposition is naturally suited to unit vectors. Angles and radii are a meaningful
parameterization for points on the sphere. The analytic codebooks from the paper are not just
principled; they actually beat our initial empirically fitted ones (V1 $\to$ V2 gain on T1: +16\%),
which suggests the theoretical priors are better than what you get from fitting to CLIP specifically.

\paragraph{Why doesn't TurboQuant beat PolarQuant?}
The short answer is that at moderate bit budgets (2--4\,bits), the 1-bit overhead for QJL costs more
than the unbiased correction gains. At 4\,bits, PolarQuant already reconstructs most of the signal, so
the residual that QJL is correcting is tiny, but QJL still spends one full bit per dimension on it, and
that variance shows up as rank perturbations in cross-modal retrieval. Retrieval is a ranking problem,
not an MSE problem, and a small unbiased correction that also adds variance can hurt top-$k$ rankings
even when it improves mean squared error.
...
Flickr30k. The central result is that cross-modal retrieval degrades far faster under compression than
...
quantization noise big enough to be negligible within a modality can still cross the inter-modal
boundary.

Among the compressors: FAISS-PQ and PolarQuant are the strongest at 2--4\,bits. If you cannot afford
training data or a codebook rebuild when the database changes, PolarQuant is the best option. It
matches FAISS-PQ at 4\,bits and gets to 80\% cross-modal retention at 2\,bits without seeing a single
...
Finally, our V1-to-V2 journey shows that getting the algorithm right matters more than tuning. Going
from the MSE stage to PolarQuant in TurboQuant, and from Gaussian projection to SRHT in QJL, doubled
cross-modal recall at 2\,bits without any hyperparameter search. Closer implementation fidelity to the
papers (analytic codebooks, correct angle domains, proper seed splitting) accounted for all of that
gain.
...
\begin{itemize}[itemsep=0pt]
  \item \textbf{Code repository:} \url{https://github.com/QUASARS06/turboquant-clip}
  \item \textbf{Embedding notebook (Colab):} \texttt{PASTE\_COLAB\_LINK\_HERE}
  \item \textbf{Results/figures:} \texttt{PASTE\_RESULTS\_LINK\_HERE}
\end{itemize}

\paragraph{Reproducibility.}
The full experiment can be reproduced with:
\begin{verbatim}
python -m src.embed encode --out-dir data
...
This appendix contains the prompts and key responses used to assist with implementation design,
debugging, and report drafting. All final code and experimental conclusions were independently verified
through local tests and rerun experiments on our dataset.

\textit{[Prompts and responses to be filled in manually by team members.]}
