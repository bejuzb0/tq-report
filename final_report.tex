...
no index rebuild, making large-scale image search practical on commodity hardware. This would enable
real-time cross-modal search over billion-scale collections on a single server, without GPU-resident
indexes, a prerequisite for cost-sensitive deployments like product search or content moderation.
...
\textbf{H2:} The analytic compressors can compress CLIP embeddings to 4\,bits and retain
cross-modal Recall@10 within 5 percentage points of FAISS-PQ at the same bit budget.
...
Our work uses the same compressors and retrieval framework but differs in two ways: the embedding space
is multimodal and the tasks are cross-modal. Existing evaluations use same-modal benchmarks that cannot
detect whether compression damages inter-modal alignment. No prior work measures whether the modality
gap survives quantization.
...
and 155{,}070 captions (five per image). Its 31k-image scale is large enough to expose quantization
artifacts while remaining tractable for CPU-resident experiments. We use CLIP ViT-B/32: its
512-dimensional embeddings fit the entire database in RAM and are the most widely deployed CLIP variant.
Embeddings are L2-normalized to unit norm (required by PolarQuant's polar decomposition). All embeddings
were computed once on a T4 GPU (Google Colab); compression and retrieval ran on CPU (MacBook Pro M1 Pro,
16\,GB RAM).
...
\paragraph{H2: Analytic compressors within 5\,pp of FAISS-PQ at 4\,bits (confirmed).}
At 4\,bits, PolarQuant retains 98.4\%/98.1\% of cross-modal recall (T1/T2) vs.\ FAISS-PQ's
99.0\%/99.3\% — a gap of just 0.6--1.2 percentage points, well within the 5\,pp threshold. TurboQuant
retains 94.6\%/95.7\%, a gap of 3.6--4.4 pp, also within threshold. H2 is confirmed for both methods.
TurboQuant does not beat PolarQuant at any bit-width because it always reserves 1 bit for the QJL
residual, leaving only $b{-}1$ bits for the polar stage; it pays off at 4\,bits and is substantially
better than QJL alone (+25--48 points on T1 at 2--4\,bits).
