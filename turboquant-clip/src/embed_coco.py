"""Precompute CLIP ViT-B/32 embeddings for MSCOCO. Resumable + chunked.
...
Resumability
------------
Embeddings are saved as small chunk files in `data_coco/chunks/images/` and
`data_coco/chunks/texts/`. On each run, the script scans which chunk indices
...
`finalize` step concatenates chunks into two single `.npy` files:
    ...
    data_coco/image_embeddings.npy      # (5000, 512)  L2-normalized float32
    data_coco/text_embeddings.npy       # (25000, 512) L2-normalized float32
    data_coco/captions.json             # list of 25000 caption strings
    data_coco/image_ids.json            # list of 5000 stable image IDs
...
-----
    ...
    python -m src.embed_coco encode --out-dir /content/drive/MyDrive/cs639/data_coco
    # If the session dies, re-run the same command — it will pick up where it left off.

    # After all chunks are saved:
    python -m src.embed_coco finalize --out-dir /content/drive/MyDrive/cs639/data_coco
...
def encode(out_dir: Path) -> None:
    ...
    if not meta_path.exists() or not ids_path.exists():
        ...
        image_ids: list[str] = []
        for row in ds:
            image_ids.append(str(row["cocoid"]))
            caps = row["caption"]
...
def main():
    ...
    p.add_argument("--out-dir", default="data_coco", type=Path)
    args = p.parse_args()
