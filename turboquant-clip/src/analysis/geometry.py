...
def run(args):
    ...
    print(stats)
    (out_dir / f"{data_dir.name}_geometry_stats.txt").write_text(str(stats))
    ...
    fig.tight_layout()
    fig.savefig(out_dir / f"{data_dir.name}_fig_geometry.png", dpi=150)
    print(f"[geometry] wrote {out_dir / f'{data_dir.name}_fig_geometry.png'}")
