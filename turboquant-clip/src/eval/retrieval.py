...
def _unc_recall_at_k(
    comp_scores: np.ndarray,   # (nq, N) — already masked if needed
    unc_scores:  np.ndarray,   # (nq, N) — same masking applied
    k: int = 10,
) -> float:
    """Mean fraction of uncompressed top-k items recovered by compressed top-k."""
    nq = comp_scores.shape[0]
    unc_topk  = np.argpartition(-unc_scores,  k, axis=1)[:, :k]
    comp_topk = np.argpartition(-comp_scores, k, axis=1)[:, :k]
    hits = sum(len(np.intersect1d(unc_topk[i], comp_topk[i])) for i in range(nq))
    return hits / (nq * k)
...
def evaluate_all_tasks(
    ...
    seed: int = 0,
    return_unc_recall: bool = False,
) -> "list[TaskResult] | tuple[list[TaskResult], list[TaskResult]]":
    ...
    if return_unc_recall:
        unc_s = q @ image_db.T
        unc_results.append(TaskResult("T1_text2image",
                                      {10: _unc_recall_at_k(scores, unc_s)}, len(gt)))
    ...
    if return_unc_recall:
        unc_s = q @ text_db.T
        unc_results.append(TaskResult("T2_image2text",
                                      {10: _unc_recall_at_k(scores, unc_s)}, len(gt)))
    ...
    if return_unc_recall:
        unc_s = q @ text_db.T
        for row, j in enumerate(txt_q_idx):
            unc_s[row, int(j)] = -np.inf
        unc_results.append(TaskResult("T3_text2text",
                                      {10: _unc_recall_at_k(scores, unc_s)}, len(gt)))
    ...
    if return_unc_recall:
        # gold_scores is already the full-precision image scores with self excluded
        unc_results.append(TaskResult("T4_image2image",
                                      {10: _unc_recall_at_k(scores, gold_scores)}, len(gt)))

    if return_unc_recall:
        return results, unc_results
    return results
